...
<ComparisonTable
  ...
  rows={[
    ...
    { feature: "No Per-Search Metering", values: [true, true] },
...
XTAL is also custom-quoted and contact-for-pricing. It does not use a per-search metering model, which means costs stay predictable as traffic grows — you're not watching a usage counter during peak season. Full pricing details are available on request.

**Bottom line for budget planning:** If you are evaluating on cost, the most useful thing you can do is request a locked-price, three-year projection from both vendors that includes your expected session growth, any tier upgrades, and add-ons. Searchspring renewals have historically surprised customers on the upside; XTAL's flat-rate model removes that specific risk. The [Searchspring comparison page](/compare/searchspring) has more detail on the contract structure.
...
**You're hitting a pricing wall.** If your current Searchspring bill has grown faster than your perceived value from the platform, or if you're facing a renewal quote that feels disconnected from results, this is the right moment to benchmark alternatives. XTAL's flat-rate model avoids the session and SKU overages that drive Searchspring costs up over time.
...
**You're evaluating from scratch and Searchspring is on the list.** For a net-new evaluation with no prior Searchspring investment, XTAL's setup speed, AI-native pipeline, and lack of metering overages make it a strong candidate for mid-market stores in the $5M–$100M revenue range.
