...
faqItems:
  ...
  - question: "Does XTAL have an equivalent to Algolia Recommend?"
    answer: "XTAL's aspects and reasoning features serve a similar purpose — surfacing related and complementary products based on semantic understanding rather than collaborative filtering alone. XTAL also has an early-stage recommendations API (currently powering cannabis dispensary recommendations) with an architecture designed to extend to other verticals."
---
...
<ComparisonTable
  ...
  rows={[
    ...
    { feature: "Merchandising controls", values: ["AI-guided via prompts", "Visual rules engine"] },
    { feature: "Context-aware personalization", values: ["Geo + seasonal (zero-config)", "Behavioral (Premium+)"] },
