...
const XTAL_LOGO_SVG = `<svg viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg" width="32" height="32">
  <rect x="8" y="6" width="8" height="36" rx="4" fill="white" transform="rotate(-45 24 24)"/>
  <rect x="8" y="6" width="8" height="36" rx="4" fill="white" transform="rotate(45 24 24)"/>
</svg>`
...
function renderStars(rating: number | undefined): string {
  ...
  return (
    '<span style="color:#FFB800;font-size:13px;letter-spacing:-1px;">' +
    "★".repeat(full) +
    (half ? "⯨" : "") +
    '<span style="color:#ccc;">' +
    "★".repeat(empty) +
    "</span>" +
    `<span style="color:#888;font-size:11px;margin-left:3px;">${rating.toFixed(1)}</span>` +
    "</span>"
  )
}
...
function merchantResultCard(r: MerchantResult, idx: number): string {
  const img = r.imageUrl
    ...
    : `<div style="width:80px;height:80px;border-radius:6px;background:#f0f0f0;flex-shrink:0;display:flex;align-items:center;justify-content:center;color:#aaa;font-size:11px;">No img</div>`

  return `<div style="display:flex;gap:10px;align-items:flex-start;padding:8px 0;${idx > 0 ? "border-top:1px solid #eee;" : ""}">
    ${img}
    <div style="min-width:0;flex:1;">
      <div style="font-size:13px;font-weight:500;color:#1a1a1a;line-height:1.3;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden;">${escapeHtml(truncate(r.title, 80))}</div>
      <div style="margin-top:4px;">
        ${r.price ? `<span style="font-size:14px;font-weight:700;color:#1a1a1a;">${formatPrice(r.price)}</span>` : ""}
      </div>
      ${r.rating ? `<div style="margin-top:2px;">${renderStars(r.rating)}</div>` : ""}
    </div>
  </div>`
}
...
<div style="width:${SLIDE_W}px;height:${SLIDE_H}px;display:flex;flex-direction:column;">
  ...
  <!-- Header -->
  <div style="background:#0F1A35;padding:28px 36px 20px;">
    ...
    <div style="color:white;font-size:28px;font-weight:700;line-height:1.2;margin-bottom:10px;">"${escapeHtml(truncate(comparison.query, 60))}"</div>
  ...
  <div style="display:flex;border-bottom:2px solid #eee;">
    <div style="flex:1;padding:14px 24px;display:flex;align-items:center;gap:8px;border-right:1px solid #eee;">
      ...
      <span style="font-size:12px;color:#999;margin-left:auto;">${comparison.merchant.resultCount} results · ${Math.round(comparison.merchant.responseTime)}ms</span>
    </div>
    <div style="flex:1;padding:14px 24px;display:flex;align-items:center;gap:8px;">
      ...
      <span style="font-size:12px;color:#999;margin-left:auto;">${comparison.xtal.resultCount} results · ${Math.round(comparison.xtal.responseTime)}ms</span>
    </div>
  </div>
  ...
  <div style="display:flex;flex:1;overflow:hidden;">
    <!-- Merchant Column -->
    <div style="flex:1;padding:12px 24px;border-right:1px solid #eee;">
      ${merchantError}
      ${merchantEmpty}
      ${merchantResults.map((r, i) => merchantResultCard(r, i)).join("")}
    </div>
    <!-- XTAL Column -->
    <div style="flex:1;padding:12px 24px;background:rgba(27,45,91,0.02);">
      ${xtalEmpty}
  ...
  <!-- Footer -->
  <div style="background:#0F1A35;padding:12px 36px;display:flex;align-items:center;justify-content:space-between;">
...
<div style="width:${SLIDE_W}px;height:${SLIDE_H}px;background:linear-gradient(155deg,#0F1A35 0%,#1B2D5B 50%,#2A4080 100%);display:flex;flex-direction:column;justify-content:center;align-items:center;text-align:center;padding:60px;">
  ...
  <div style="color:white;font-size:56px;font-weight:900;line-height:1.1;margin-bottom:32px;">${escapeHtml(merchant.name)}</div>

  <div style="color:rgba(255,255,255,0.7);font-size:20px;font-weight:400;line-height:1.5;max-width:700px;margin-bottom:48px;">
    ${totalQueries} real shopper queries.<br>
    Their search vs. AI-powered search.<br>
    Side by side.
  </div>
...
<div style="width:${SLIDE_W}px;height:${SLIDE_H}px;background:linear-gradient(155deg,#0F1A35 0%,#1B2D5B 50%,#2A4080 100%);display:flex;flex-direction:column;justify-content:center;align-items:center;text-align:center;padding:60px;">
  ...
  <div style="color:white;font-size:44px;font-weight:800;line-height:1.15;max-width:800px;margin-bottom:40px;">
    Is your site search<br>leaving money on the table?
  </div>

  <div style="color:rgba(255,255,255,0.6);font-size:20px;font-weight:400;line-height:1.6;max-width:650px;margin-bottom:48px;">
    Get a free AI-powered search health report for your store in under 60 seconds.
  </div>

  <div style="background:white;color:#1B2D5B;font-size:20px;font-weight:700;padding:18px 48px;border-radius:12px;margin-bottom:16px;">
    xtalsearch.com/grade
  </div>

  <div style="color:rgba(255,255,255,0.4);font-size:14px;margin-top:8px;">Free · No signup required · 60-second analysis</div>

  <div style="position:absolute;bottom:48px;display:flex;align-items:center;gap:10px;">
