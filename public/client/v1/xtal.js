...
`,document.head.appendChild(r)}function Se(){return/[?&](Search|search)=/.test(window.location.search)}function ke(r){if(document.getElementById("xtal-sdk-early-hide"))return;let e=document.createElement("style");e.id="xtal-sdk-early-hide",e.textContent=`${r} { visibility: hidden !important; min-height: calc(100vh - 160px); }`,document.head.appendChild(e)}function de(){try{let r=document.querySelector("script[data-shop-id]"),e=window.XTAL_CONFIG,n=r?.getAttribute("data-shop-id")||e?.shopId||"";if(!n){console.warn("[xtal.js] No shopId found \u2014 need <script data-shop-id> or window.XTAL_CONFIG = { shopId: '...' }");return}let t="",o=r?.getAttribute("src");if(o)try{t=new URL(o,window.location.href).origin}catch{}t||(t=e?.apiBase||"https://www.xtalsearch.com");let s=new U(t,n),p=3e5,d=864e5,h=`xtal:config:${n}`,a=null,u=null;try{let l=localStorage.getItem(h);if(l){let m=JSON.parse(l),b=Date.now()-m.ts;b<p?a=m.config:b<d&&m.config.resultsSelector&&(u=m.config.resultsSelector)}}catch{}if(window.__XTAL_LOADING=!0,Se()){let l=a?.resultsSelector||u||e?.resultsSelector;l&&!ce.has(l.trim().toLowerCase())&&ke(l)}let i=l=>{try{localStorage.setItem(h,JSON.stringify({config:l,ts:Date.now()}))}catch{}},c=l=>{if(!l.enabled){console.log(`[xtal.js] Snippet disabled for ${n}`);return}let m=l.cardTemplate??null;if(m?.css){let v=document.getElementById("xtal-card-styles");v&&v.remove();let y=document.createElement("style");y.id="xtal-card-styles",y.textContent=Te(m.css,".xtal-layout"),document.head.appendChild(y)}function b(v){if(l.productUrlPattern){let S=v.variants?.[0]?.sku||"";if(S){let L=l.productUrlPattern.replace("{sku}",encodeURIComponent(S)).replace("{id}",v.id||"");if(!/^javascript:/i.test(L)&&!/^data:/i.test(L))return L}}let y=v.product_url||"#";return!y||y==="#"?"#":y.startsWith("http://")||y.startsWith("https://")?y:l.siteUrl?l.siteUrl.replace(/\/$/,"")+y:y}let E="",T=le(n,()=>E,b);console.log(`[xtal.js] Cart adapter: ${T.name}`);let C=l.resultsSelector??"",w=!!C&&!ce.has(C.trim().toLowerCase());if(!(l.displayMode==="inline"&&w)){!w&&C&&console.warn(`[xtal.js] resultsSelector "${C}" is blocked \u2014 SDK disabled`),window.XTAL={destroy(){let v=document.getElementById("xtal-card-styles");v&&v.remove(),window.XTAL=void 0}};return}{let v=document.querySelector(C),y=v?new B(v):null,S=null,L=()=>y||(v=document.querySelector(C),v&&(y=new B(v),H&&y.initLayout()),y),H=l.features?.filters===!0,I=null,k={},P=null,_=null,O=0,z={},R=null;H&&(Le(),y?.initLayout());let pe=()=>{if(_||!H||!y)return;let g=y.initLayout();_=new X(g,(f,$)=>{k[f]||(k[f]=[]);let ie=k[f].indexOf($);ie>=0?(k[f].splice(ie,1),k[f].length===0&&delete k[f]):k[f].push($),G()},f=>{P=f,G()},()=>{k={},P=null,G()},l.pricePresets,l.hiddenFacets)},Y={onViewProduct(g){let f=N(b(g),{shopId:n,productId:g.id,query:E});window.open(f,"_blank","noopener,noreferrer")},async onAddToCart(g){let f=await T.addToCart(g);console.log(`[xtal.js] Add to cart: ${f.success?"OK":"FAIL"} \u2014 ${f.message}`),f.success&&fetch(`${t}/api/xtal/events`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({product_id:g.id,action:"add_to_cart",collection:n,query:E})}).catch(()=>{})}},ee=g=>g.map(f=>m?q(m.html,f,E,n,Y,T.name,b(f)):se(f,E,n,null,Y,b(f))),G=()=>{!I||!y||(R&&clearTimeout(R),R=setTimeout(()=>{y?.showFilterLoading(),s.searchFiltered(E,I,{facetFilters:k,priceRange:P,limit:l.resultsPerPage??24}).then(g=>{O=g.total,z=g.computed_facets||{},g.results.length===0?y?.renderEmpty(E):y?.renderCards(ee(g.results)),_?.update(z,k,P,O)}).catch(g=>{g instanceof DOMException&&g.name==="AbortError"||(console.error("[xtal.js] Filter error:",g),Z(t,n,String(g),"filter"))})},350))},D=l.siteUrl&&(l.siteUrl.startsWith("https://")||l.siteUrl.startsWith("http://"))?l.siteUrl.replace(/\/$/,""):"",V=g=>{if(E=g,!L()){let f=/[?&](Search|search)=/.test(window.location.search);D&&!f&&(console.log("[xtal.js] No results container \u2014 navigating to search page"),window.location.href=`${D}/shop/?Search=${encodeURIComponent(g)}`);return}I=null,k={},P=null,_?.closeDrawer(),_?.resetState(),y.showLoading(g),s.searchFull(g,l.resultsPerPage??24).then(f=>{O=f.total,z=f.computed_facets||{},I=f.search_context||null,pe();let $=document.querySelector(J);if($&&$.value!==g&&($.value=g),f.results.length===0){y?.renderEmpty(g),_?.update({},{},null,0);return}y?.renderCards(ee(f.results)),_?.update(z,k,P,O)}).catch(f=>{f instanceof DOMException&&f.name==="AbortError"||(console.error("[xtal.js] Search error:",f),Z(t,n,String(f),"search"),y?.restore(),D&&E&&(window.location.href=`${D}/shop/?Search=${encodeURIComponent(E)}`))})},j=null,te=g=>{j&&clearTimeout(j),j=setTimeout(()=>V(g),200)},J=l.searchSelector||'input[type="search"]';S=oe(J,te,l.observerTimeoutMs);let F=null;v||(console.log(`[xtal.js] Inline mode: "${C}" not found \u2014 watching`),F=new MutationObserver(()=>{L()&&(F?.disconnect(),F=null,E&&!I&&V(E))}),F.observe(document.body,{childList:!0,subtree:!0}),setTimeout(()=>{F?.disconnect(),F=null},l.observerTimeoutMs??1e4));let ne=new URLSearchParams(window.location.search),re=ne.get("Search")||ne.get("search");re?.trim()&&V(re.trim()),window.XTAL={search(g){g?.trim()&&te(g.trim())},destroy(){j&&clearTimeout(j),R&&clearTimeout(R),s.abort(),S?.(),F?.disconnect(),_?.destroy(),y?.destroy();let g=document.getElementById("xtal-card-styles");g&&g.remove();let f=document.getElementById("xtal-filter-styles");f&&f.remove(),window.XTAL=void 0}},console.log(`[xtal.js] Initialized INLINE for ${n}. Search: ${J}, Grid: ${l.resultsSelector}${v?"":" (deferred)"}${H?", Filters: ON":""}`)}window.__XTAL_LOADING=!1},x=!1;a?(c(a),x=!0,s.fetchConfig().then(i).catch(()=>{})):s.fetchConfig().then(l=>{i(l),x||c(l)}).catch(l=>{console.error("[xtal.js] Failed to fetch config:",l),Z(t,n,String(l),"config"),window.__XTAL_LOADING=!1})}catch(r){console.error("[xtal.js] Boot error:",r)}}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",de):de();})();
