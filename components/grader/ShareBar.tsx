...
export default function ShareBar({ reportId, storeName, score }: ShareBarProps) {
  ...
  const handleDownloadPdf = useCallback(async () => {
    ...
    try {
      const res = await fetch(`/api/grader/report/${reportId}/pdf`)
      ...
      a.href = url
      a.download = `${storeName.replace(/[^a-zA-Z0-9]/g, "-").toLowerCase()}-search-report.pdf`
  ...
  const btnClass =
    "inline-flex items-center gap-2 px-5 py-2.5 rounded-full border border-slate-200 text-sm font-medium text-slate-600 hover:border-xtal-navy/30 hover:text-xtal-navy transition-colors"
  ...
  return (
    <div className="flex flex-wrap items-center justify-center gap-3 py-6">
      ...
      <button
        ...
        className={`${btnClass} ${downloading ? "opacity-60 cursor-wait" : ""}`}
      >
        {downloading ? (
          <>
