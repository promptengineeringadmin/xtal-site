...
export async function GET(request: Request) {
  try {
    ...
    console.error("Marketing prompt GET proxy error:", error)
    return NextResponse.json({ error: "Failed to fetch marketing prompt" }, { status: 502 })
  }
}
...
export async function PUT(request: Request) {
  try {
    ...
    const payload = { marketing_prompt: body.marketing_prompt, collection }
    ...
    console.error("Marketing prompt PUT proxy error:", error)
    return NextResponse.json({ error: "Failed to update marketing prompt" }, { status: 502 })
  }
}
