...
/* POST persists a completed optimization run to the backend events system */
export async function POST(request: Request) {
  try {
    ...
    // History persistence failure is non-critical
    return NextResponse.json(
      { error: `Failed to persist optimization result: ${err.message}` },
      { status: 502 }
    )
  }
}
